...
fn default_video_background_enabled() -> bool {
    true
}
fn default_video_canvas() -> String {
    "landscape".into()
}
fn default_video_audio_mode() -> String {
    "with-audio".into()
}
...
fn portrait_metadata_path() -> Result<PathBuf, String> {
    Ok(video_root()?.join("portrait.json"))
}
fn background_metadata_path() -> Result<PathBuf, String> {
    Ok(video_root()?.join("background.json"))
}
fn video_job_path(id: &str) -> Result<PathBuf, String> {
    Ok(video_jobs_root()?.join(format!("{id}.json")))
}

fn write_video_job(job: &VideoJob) -> Result<(), String> {
    fs::write(
        video_job_path(&job.id)?,
...
fn wav2lip_paths() -> Result<(PathBuf, PathBuf, PathBuf), String> {
    ...
    Ok((
        root.clone(),
        root.join("checkpoints/wav2lip_gan.pth"),
        root.join("face_detection/detection/sfd/s3fd.pth"),
    ))
}
...
fn update_job(id: &str, mutate: impl FnOnce(&mut SynthesisJob)) -> Result<SynthesisJob, String> {
    let mut job: SynthesisJob =
        serde_json::from_slice(&fs::read(job_path(id)?).map_err(|error| error.to_string())?)
            .map_err(|error| error.to_string())?;
    mutate(&mut job);
    write_job(&job)?;
    Ok(job)
...
fn current_device_profile() -> DeviceProfile {
    ...
    let (performance_tier, model, chunk_characters, system_concurrency) = if memory_gb >= 24 {
        // Longer semantic windows preserve prosody across a long narration.
        // The 1.7B model has enough memory headroom on the quality tier.
        (
            "quality",
...
#[tauri::command]
fn public_voice_availability(
    voices: Vec<PublicVoiceRequest>,
) -> Result<HashMap<String, bool>, String> {
    let models = app_root()?.join("models");
    let qwen = models.join(open_voice_model().replace('/', "--"));
    let qwen_ready = qwen.join("model.safetensors").exists()
        && qwen.join("speech_tokenizer/model.safetensors").exists();
        ...
        .map(|voice| {
            let ready = if voice.id.starts_with("qwen-") {
                ...
                kokoro_ready
                    && kokoro
                        .join("voices")
                        .join(format!("{}.safetensors", voice.speaker))
                        .exists()
            } else {
                false
            };
...
#[tauri::command]
async fn download_public_voice(
    app: tauri::AppHandle,
    voice_id: String,
    speaker: String,
) -> Result<(), String> {
    if !(voice_id.starts_with("qwen-") || voice_id.starts_with("kokoro-"))
        || !speaker
            .chars()
    ...
    let model_id = if voice_id.starts_with("kokoro-") {
    ...
    let temporary = app_root()?
        .join("temp")
        .join(format!("voice-download-{}", Uuid::new_v4()));
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        ...
        let china_timezone = fs::read_link("/etc/localtime")
            .map(|path| {
                path.to_string_lossy().contains("Asia/Shanghai")
                    || path.to_string_lossy().contains("Asia/Chongqing")
                    || path.to_string_lossy().contains("Asia/Hong_Kong")
            })
            .unwrap_or(false);
        if china_timezone {
            command.env("VOXLOCAL_MODEL_SOURCE", "modelscope");
        }
    ...
    if result.status.success() {
        ...
        Err(String::from_utf8_lossy(&result.stdout)
            .lines()
            .last()
            .unwrap_or("音色下载失败")
            .to_owned())
...
fn engine_status() -> EngineStatus {
    ...
    let multilingual_runtime_ready = runtime_root
        .as_ref()
        .map(|path| path.join(".kokoro-multilingual-ready").exists())
        .unwrap_or(false);
    let model_directory = app_root().ok().map(|root| {
        root.join("models")
            .join(profile.recommended_model.replace('/', "--"))
    });
    let model_ready = model_directory
        .as_ref()
        .map(|path| {
            path.join("model.safetensors").exists()
                && path.join("speech_tokenizer/model.safetensors").exists()
        })
        .unwrap_or(false);
    EngineStatus {
        supported,
...
async fn prepare_ai_engine(app: tauri::AppHandle) -> Result<EngineStatus, String> {
    ...
    if !venv.join("bin/python").exists() {
        let result = Command::new(&uv)
            ...
            .map_err(|error| error.to_string())?;
        if !result.success() {
            return Err("创建本地 Python 运行时失败".into());
        }
    }
    ...
    let result = Command::new(&uv)
        ...
        .map_err(|error| error.to_string())?;
    if !result.success() {
        return Err("安装 MLX 音频引擎失败".into());
    }
    let english_ready = Command::new(&python)
    ...
    if !japanese_ready {
        return Err("安装日语发音词典失败".into());
    }
    fs::write(
        runtime_dir.join(".kokoro-multilingual-ready"),
        b"misaki-en-ja-zh-v2\n",
    )
    .map_err(|error| error.to_string())?;
...
fn voice_name(profile: &str, language: &str) -> Option<String> {
    ...
    for hint in hints {
        if let Some(line) = listing
            .lines()
            .find(|line| line.to_lowercase().starts_with(&hint.to_lowercase()))
        {
    ...
    listing
        .lines()
        .find(|line| line.contains(locale))
...
async fn speak_text(
    ...
    if let Some(pid) = state.pid.lock().map_err(|_| "播放状态不可用")?.take() {
        let _ = Command::new("/bin/kill").arg(pid.to_string()).status();
    }
    let mut command = Command::new("/usr/bin/say");
    command.args(["-r", &words_per_minute.clamp(80, 320).to_string()]);
    if let Some(name) = voice_name(&voice_id, language.as_deref().unwrap_or("zh")) {
        command.args(["-v", &name]);
    }
    command.arg(text);
    ...
    *state.pid.lock().map_err(|_| "播放状态不可用")? = Some(pid);
    ...
    if result.success() || result.code().is_none() {
        Ok(())
    } else {
        Err(format!("本地播放失败：{result}"))
    }
}
...
fn persist_voice_recording(recording_id: String, bytes: Vec<u8>) -> Result<String, String> {
    if bytes.is_empty() || bytes.len() > 100 * 1024 * 1024 {
        return Err("录音数据无效或过大".into());
    }
    let safe_id: String = recording_id
        .chars()
        .filter(|character| character.is_ascii_alphanumeric() || *character == '-')
        .collect();
    if safe_id.is_empty() {
        return Err("录音标识无效".into());
    }
...
fn delete_voice_recording(path: String) -> Result<(), String> {
    ...
    let voices = app_root()?
        ...
        .canonicalize()
        .map_err(|error| error.to_string())?;
    if !candidate.starts_with(&voices) {
        return Err("只能删除 VoxLocal 的音色录音".into());
    }
    let stem = candidate
        .file_stem()
...
fn persist_voice_profile(profile: serde_json::Value) -> Result<(), String> {
    ...
    let safe_id: String = id
        ...
        .filter(|character| character.is_ascii_alphanumeric() || *character == '-')
        .collect();
    if safe_id != id || !id.starts_with("voice-") {
        return Err("音色标识无效".into());
    }
    let directory = app_root()?.join("voices");
    fs::create_dir_all(&directory).map_err(|error| error.to_string())?;
    fs::write(
        directory.join(format!("{safe_id}.json")),
...
fn list_voice_profiles() -> Result<Vec<serde_json::Value>, String> {
    ...
    for entry in fs::read_dir(directory).map_err(|error| error.to_string())? {
        ...
        let Ok(profile) = serde_json::from_slice::<serde_json::Value>(&data) else {
            continue;
        };
        let Some(id) = profile.get("id").and_then(|value| value.as_str()) else {
            continue;
        };
    ...
    profiles.sort_by(|left, right| {
        right
            .get("createdAt")
            .and_then(|value| value.as_str())
            .cmp(&left.get("createdAt").and_then(|value| value.as_str()))
    });
    Ok(profiles)
}
...
fn trash_voice_profile(voice_id: String) -> Result<serde_json::Value, String> {
    ...
    let mut profile: serde_json::Value =
        serde_json::from_slice(&fs::read(&metadata).map_err(|_| "找不到这个克隆音色档案")?)
            .map_err(|error| error.to_string())?;
    if let Some(recording) = profile
        .get("recordingPath")
        .and_then(|value| value.as_str())
        .map(PathBuf::from)
    {
    ...
    profile["trashedAt"] = serde_json::Value::String(format!(
        "{}",
    ...
    fs::write(
        trash.join(format!("{voice_id}.json")),
        serde_json::to_vec_pretty(&profile).map_err(|error| error.to_string())?,
    )
    .map_err(|error| error.to_string())?;
    fs::remove_file(metadata).map_err(|error| error.to_string())?;
...
fn list_trash_voice_profiles() -> Result<Vec<serde_json::Value>, String> {
    ...
    for entry in fs::read_dir(trash).map_err(|error| error.to_string())? {
        ...
        if let Ok(profile) = fs::read(&path)
            .ok()
...
fn restore_voice_profile(voice_id: String) -> Result<serde_json::Value, String> {
    ...
    let mut profile: serde_json::Value =
        serde_json::from_slice(&fs::read(&metadata).map_err(|_| "回收站中找不到这个音色")?)
            .map_err(|error| error.to_string())?;
    ...
    fs::write(
        voices.join(format!("{voice_id}.json")),
        serde_json::to_vec_pretty(&profile).map_err(|error| error.to_string())?,
    )
    .map_err(|error| error.to_string())?;
    fs::remove_file(metadata).map_err(|error| error.to_string())?;
...
fn split_text(text: &str, limit: usize) -> Vec<String> {
    ...
    // Keep commas and colons inside a chunk.  Resetting the TTS model at a
    // weak punctuation mark makes long narration restart its prosody too
    // often, which is heard as abrupt speed/timbre changes.  Only split at
    // sentence-level boundaries; an exceptionally long sentence is handled
    // by the character-count fallback below.
    for sentence in cleaned.split_inclusive(|character| {
        matches!(
            character,
            '。' | '！' | '？' | '；' | '\n' | '.' | '!' | '?' | ';'
        )
...
fn concat_wav(parts: &[PathBuf], output: &Path) -> Result<(), String> {
    ...
    for part in parts {
        ...
        let samples: Vec<i16> = reader
            ...
            .map_err(|error| error.to_string())?;
        if !joined.is_empty() {
            joined.extend(std::iter::repeat(0i16).take(pause_samples));
        }
...
fn generate_system_parts(
    ...
    let resolved_voice = voice_name(
        &request.voice_id,
        request.language.as_deref().unwrap_or("zh"),
    );
    let concurrency = current_device_profile().system_concurrency.max(1);
    let parts: Vec<PathBuf> = (0..chunks.len())
        .map(|index| temp.join(format!("part-{index:05}.wav")))
        .collect();
    for batch_start in (0..chunks.len()).step_by(concurrency) {
        ...
        std::thread::scope(|scope| -> Result<(), String> {
            ...
            for index in batch_start..batch_end {
                ...
                handles.push(scope.spawn(move || -> Result<(), String> {
                    ...
                    let status = command
                        ...
                        .map_err(|error| error.to_string())?;
                    if !status.success() {
                        return Err(format!("第 {} 段系统语音生成失败", index + 1));
                    }
                    let status = Command::new("/usr/bin/afconvert")
                        .args(["-f", "WAVE", "-d", "LEI16@24000", "-c", "1"])
                        .arg(&aiff)
            ...
            for handle in handles {
                handle
                    .join()
                    .map_err(|_| "系统语音工作线程异常退出".to_owned())??;
            }
            Ok(())
        })?;
        let percent = ((batch_end as f32 / chunks.len() as f32) * 88.0).round() as u8;
        update_job(job_id, |job| {
            job.stage = format!("已生成 {}/{} 段", batch_end, chunks.len());
            job.progress = percent;
            job.completed_chunks = batch_end;
        })?;
    }
    Ok(parts)
}
...
fn engine_script(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    if let Ok(resource) = app.path().resource_dir() {
        for candidate in [
            resource.join("backend/voxlocal_engine.py"),
            resource.join("voxlocal_engine.py"),
        ] {
            if candidate.exists() {
                return Ok(candidate);
            }
    ...
    let development =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../backend/voxlocal_engine.py");
    if development.exists() {
        Ok(development)
    } else {
        Err("找不到本地模型执行程序".into())
    }
}
...
fn sync_bundled_voice_previews(app: &tauri::AppHandle) -> Result<(), String> {
    let mut candidates =
        vec![PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../assets/public-voice-previews")];
    if let Ok(resource) = app.path().resource_dir() {
...
fn generate_mlx_parts(
    ...
    let is_preset_voice =
        request.voice_id.starts_with("qwen-") || request.voice_id.starts_with("kokoro-");
    if !is_preset_voice {
        ...
        if !Path::new(reference).exists() {
            return Err("克隆音色的参考录音不存在".into());
        }
    }
    let python = runtime_python()?;
    if !python.exists() {
        return Err("请先安装本地 MLX 引擎".into());
    }
    let profile = current_device_profile();
    let parts: Vec<PathBuf> = (0..chunks.len())
        .map(|index| temp.join(format!("part-{index:05}.wav")))
    ...
    let items: Vec<serde_json::Value> = chunks
        .iter()
        .zip(parts.iter())
        .map(|(text, output)| serde_json::json!({"text": text, "output": output}))
        .collect();
    ...
    command
        ...
        .args(["--request", request_path.to_string_lossy().as_ref()]);
    if china_timezone {
        command.env("VOXLOCAL_MODEL_SOURCE", "modelscope");
    }
    ...
    let event_result = std::thread::scope(|scope| {
            ...
            for line in BufReader::new(stdout).lines() {
                ...
                match event["type"].as_str() {
                    ...
                    Some("chunk_complete") => {
                        ...
                        let percent =
                            20 + ((completed as f32 / chunks.len() as f32) * 68.0).round() as u8;
                        update_job(job_id, |job| {
                    ...
                    Some("model_progress") => {
                        ...
                        let message = event["message"]
                            .as_str()
                            .unwrap_or("正在准备本地克隆模型")
                            .to_owned();
                        update_job(job_id, |job| {
                            job.stage = message;
                            job.progress = 2 + (download as f32 * 0.18).round() as u8;
                        })?;
                    }
                    Some("error") => {
        ...
        reader
            .join()
            .map_err(|_| "模型进度读取线程异常退出".to_owned())?
    });
    if cancelled.load(Ordering::Relaxed) {
        let _ = child.wait();
        return Err("__cancelled__".into());
...
fn start_synthesis(
    ...
    if request.text.trim().is_empty() {
        return Err("没有可生成的文字".into());
    }
    if request.text.chars().count() > 2_000_000 {
        return Err("单个任务最多支持 200 万字".into());
    }
    let profile = current_device_profile();
    let chunks = split_text(&request.text, profile.chunk_characters);
    let id = Uuid::new_v4().to_string();
    let uses_mlx = request.voice_id.starts_with("voice-")
        || request.voice_id.starts_with("qwen-")
        || request.voice_id.starts_with("kokoro-");
    let source = if request.title.ends_with("试听") || request.title.contains("-试听") {
        "preview"
    } else {
        "text"
    };
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let result = (|| -> Result<String, String> {
            ...
            update_job(&id, |job| {
                job.stage = "正在无缝拼接音频".into();
                job.progress = 92;
            })?;
            let output =
                exports_root()?.join(format!("{}-{}.wav", safe_stem(&request.title), &id[..8]));
...
fn get_synthesis_job(job_id: String) -> Result<SynthesisJob, String> {
    serde_json::from_slice(&fs::read(job_path(&job_id)?).map_err(|_| "找不到这个生成任务")?)
        .map_err(|error| error.to_string())
}
...
fn list_synthesis_jobs() -> Result<Vec<SynthesisJob>, String> {
    ...
    for entry in fs::read_dir(jobs_root()?).map_err(|error| error.to_string())? {
        ...
        if let Ok(job) = fs::read(&path)
            .ok()
            .and_then(|data| serde_json::from_slice::<SynthesisJob>(&data).ok())
            .ok_or(())
        {
            if job.status == "trashed" {
                continue;
            }
            let missing_completed_audio = job.status == "completed"
                && job
                    .output_path
                    .as_ref()
                    .map(|output| !Path::new(output).exists())
                    .unwrap_or(true);
            if !missing_completed_audio {
                jobs.push(job);
            }
        }
    }
...
fn restore_trashed_audio(job_id: String) -> Result<SynthesisJob, String> {
    validated_job_id(&job_id)?;
    let mut job: SynthesisJob = serde_json::from_slice(
        &fs::read(job_path(&job_id)?).map_err(|_| "回收站中找不到这个音频")?,
    )
    .map_err(|error| error.to_string())?;
    if job.status != "trashed" {
        return Err("这个音频不在回收站".into());
    }
...
fn recover_interrupted_jobs() {
    ...
    for entry in entries.flatten() {
        ...
        let Ok(mut job) = serde_json::from_slice::<SynthesisJob>(&data) else {
            continue;
        };
...
fn read_audio_file(path: String) -> Result<Vec<u8>, String> {
    ...
    if !canonical.starts_with(exports) {
        return Err("只能读取 VoxLocal 生成的音频".into());
    }
...
fn cache_voice_preview(
    ...
    let exports = exports_root()?
        .canonicalize()
        .map_err(|error| error.to_string())?;
    if !source.starts_with(exports) {
        return Err("只能缓存 VoxLocal 生成的试听音频".into());
    }
...
fn reveal_audio_file(path: String) -> Result<(), String> {
    let canonical = PathBuf::from(path)
        .canonicalize()
        .map_err(|error| error.to_string())?;
    let registered = list_synthesis_jobs()?
        .into_iter()
        .chain(list_trashed_audio_jobs()?)
        .filter_map(|job| job.output_path)
        .filter_map(|output| PathBuf::from(output).canonicalize().ok())
        .any(|output| output == canonical);
    if !registered {
        return Err("只能显示 VoxLocal 已登记的音频".into());
    }
...
fn video_engine_status() -> Result<VideoEngineStatus, String> {
    ...
    let wav2lip_ready = ffmpeg_ready
        && root.join("inference.py").is_file()
        && checkpoint.is_file()
        && detector.is_file()
    ...
    Ok(VideoEngineStatus {
        ffmpeg_ready,
        wav2lip_ready,
        wav2lip_root: Some(root.to_string_lossy().into_owned()),
        message,
    })
}
...
fn get_video_portrait() -> Result<Option<PortraitAsset>, String> {
    ...
    let asset: PortraitAsset =
        serde_json::from_slice(&fs::read(metadata).map_err(|error| error.to_string())?)
            .map_err(|error| error.to_string())?;
...
fn delete_video_background() -> Result<(), String> {
    if let Some(asset) = get_video_background()? {
...
fn persist_video_portrait(file_name: String, bytes: Vec<u8>) -> Result<PortraitAsset, String> {
    let extension = image_extension_and_validity(&file_name, &bytes)?;
    if let Some(previous) = get_video_portrait()? {
...
fn delete_video_portrait() -> Result<(), String> {
    if let Some(asset) = get_video_portrait()? {
        let _ = fs::remove_file(asset.path);
    }
...
fn persist_video_audio(file_name: String, bytes: Vec<u8>) -> Result<ImportedAudioAsset, String> {
    ...
    let extension = Path::new(&file_name)
        ...
        .unwrap_or("")
        .to_ascii_lowercase();
    if !matches!(extension.as_str(), "wav" | "mp3" | "m4a" | "aac") {
        return Err("仅支持 WAV、MP3、M4A 或 AAC 音频".into());
    }
    ...
    if !valid {
        return Err("音频扩展名与实际内容不一致".into());
    }
    let target = video_imports_root()?.join(format!(
        "{}-{}.{}",
        safe_stem(
            Path::new(&file_name)
                .file_stem()
...
fn registered_video_audio(request: &VideoRequest) -> Result<PathBuf, String> {
    let candidate = PathBuf::from(&request.audio_path)
        .canonicalize()
        .map_err(|error| error.to_string())?;
    let imported = video_imports_root()?
        .canonicalize()
        .map_err(|error| error.to_string())?;
    if candidate.starts_with(imported) {
        return Ok(candidate);
    }
    let id = request
        .synthesis_job_id
        .as_ref()
        .ok_or("音频不是已登记的 VoxLocal 资源")?;
...
fn registered_background(path: &str) -> Result<PathBuf, String> {
    ...
    if active == candidate {
        ...
        Err("只能使用当前维护的背景图片".into())
    }
}

#[tauri::command]
fn persist_ppt_slides(slides: Vec<PptSlideUpload>) -> Result<Vec<String>, String> {
    if slides.is_empty() || slides.len() > 60 {
        return Err("幻灯片数量必须在 1 到 60 页之间".into());
    }
    let root = ppt_root()?;
    ...
    for slide in slides {
        let id: String = slide
            ...
            .collect();
        if id.is_empty() || slide.bytes.len() < 100 || slide.bytes.len() > 12 * 1024 * 1024 {
            return Err("幻灯片图片无效或过大".into());
        }
...
fn media_duration(ffmpeg: &Path, path: &Path) -> Result<f64, String> {
    ...
    let executable = if probe.is_file() {
        probe
    } else {
...
fn run_ppt_video_job(
    job_id: String,
    request: PptVideoRequest,
    slides: Vec<PathBuf>,
    audio: PathBuf,
    output: PathBuf,
) {
    let result = (|| -> Result<(), String> {
        update_video_job(&job_id, |job| {
            job.status = "running".into();
            job.stage = "准备幻灯片".into();
            job.progress = 10;
        })?;
        ...
        for (index, slide) in slides.iter().enumerate() {
            ...
            let result = Command::new(&ffmpeg)
                ...
                .map_err(|error| error.to_string())?;
            if !result.status.success() {
                return Err("幻灯片画面合成失败".into());
            }
            clips.push(clip);
            update_video_job(&job_id, |job| {
                job.stage = format!("生成第 {} / {} 页", index + 1, slides.len());
                job.progress = 15 + (((index + 1) * 60) / slides.len()) as u8;
            })?;
        }
        ...
        command
            ...
            .arg(&audio)
            .args(["-map", "0:v:0"]);
        if request.audio_mode != "video-only" {
            command.args(["-map", "1:a:0", "-c:a", "aac", "-b:a", "192k"]);
        } else {
            command.arg("-an");
        }
        command
    ...
    if let Err(error) = result {
        ...
        let _ = update_video_job(&job_id, |job| {
            job.status = "failed".into();
            job.stage = "生成失败".into();
            job.error = Some(error);
        });
...
fn start_ppt_video_job(request: PptVideoRequest) -> Result<VideoJob, String> {
    if request.slide_paths.is_empty() || request.slide_paths.len() > 60 {
        return Err("请至少生成 1 页幻灯片".into());
    }
    if !matches!(request.audio_mode.as_str(), "with-audio" | "video-only") {
        return Err("不支持的导出模式".into());
    }
    let slides = request
        .slide_paths
        .iter()
        .map(|path| registered_ppt_slide(path))
        .collect::<Result<Vec<_>, _>>()?;
    let audio_request = VideoRequest {
        title: request.title.clone(),
        engine: "ppt".into(),
        portrait_path: String::new(),
        audio_path: request.audio_path.clone(),
        synthesis_job_id: request.synthesis_job_id.clone(),
        background: default_video_background(),
        background_path: None,
        background_enabled: false,
        canvas: default_video_canvas(),
        audio_mode: request.audio_mode.clone(),
    };
...
fn run_video_job(
    ...
    let result = (|| -> Result<(), String> {
        ...
        let background = if request.background.starts_with('#')
            && request.background.len() == 7
        ...
        update_video_job(&job_id, |job| {
            job.stage = if request.engine == "wav2lip" {
                "正在进行口型同步".into()
            } else {
                "正在合成画面与声音".into()
            };
        ...
        let result = if request.engine == "wav2lip" {
            ...
            if request.background_enabled && request.background_path.is_some() {
                ...
                command.args(["-y", "-loop", "1", "-i"]).arg(bg).args(["-loop", "1", "-i"]).arg(&portrait);
            } else {
                command.args(["-y", "-f", "lavfi", "-i"]).arg(format!("color=c={background}:s={size}:r=25")).args(["-loop", "1", "-i"]).arg(&portrait);
            }
            command.arg("-i").arg(&audio).args(["-filter_complex", &filter, "-c:v", "libx264", "-tune", "stillimage", "-r", "25", "-fps_mode", "cfr", "-pix_fmt", "yuv420p"]);
        ...
        if request.engine == "wav2lip" && request.background_enabled {
            if let Some(background_path) = request.background_path.as_ref() {
                ...
                let composed_result = Command::new(&ffmpeg)
                    .args(["-y", "-loop", "1", "-i"])
                    .arg(background_path)
                    .arg("-i")
                    .arg(&output)
                    ...
                    .output()
                    .map_err(|error| error.to_string())?;
                if !composed_result.status.success() {
                    return Err("背景合成失败".into());
                }
...
fn start_video_job(
    ...
    if !matches!(request.engine.as_str(), "still" | "wav2lip") {
        return Err("不支持的视频引擎".into());
    }
    if !matches!(
        request.canvas.as_str(),
        "source" | "landscape" | "square" | "portrait" | "circle"
    ) {
        return Err("不支持的视频画布".into());
    }
    if !matches!(request.audio_mode.as_str(), "with-audio" | "video-only") {
        return Err("不支持的导出模式".into());
    }
    let portrait = registered_portrait(&request.portrait_path)?;
    if request.engine == "still" && request.background_enabled && request.background_path.is_none()
    {
        return Err("请先选择背景图片，或关闭背景开关".into());
    }
    let _background = if request.engine == "still" && request.background_enabled {
        Some(registered_background(
            request.background_path.as_ref().expect("background path"),
        )?)
    } else {
        None
    };
...
fn get_video_job(job_id: String) -> Result<VideoJob, String> {
    if !job_id
        ...
        .all(|character| character.is_ascii_alphanumeric() || character == '-')
    {
        return Err("视频任务标识无效".into());
    }
    serde_json::from_slice(&fs::read(video_job_path(&job_id)?).map_err(|error| error.to_string())?)
        .map_err(|error| error.to_string())
}

#[tauri::command]
fn list_video_jobs() -> Result<Vec<VideoJob>, String> {
    let mut jobs = fs::read_dir(video_jobs_root()?)
        .map_err(|error| error.to_string())?
        .flatten()
        .filter(|entry| entry.path().extension().and_then(|value| value.to_str()) == Some("json"))
        .filter_map(|entry| fs::read(entry.path()).ok())
        .filter_map(|data| serde_json::from_slice(&data).ok())
        .collect::<Vec<VideoJob>>();
    jobs.sort_by(|left, right| right.created_at.cmp(&left.created_at));
    Ok(jobs)
}
...
fn delete_video_job(job_id: String) -> Result<(), String> {
    ...
    let path = fs::read_dir(&root)
        ...
        .find(|candidate| {
            candidate.file_stem().and_then(|value| value.to_str()) == Some(job_id.as_str())
        })
        .unwrap_or(video_job_path(&job_id)?);
    if !path.is_file() {
        return Ok(());
    }
...
fn read_video_asset(path: String, kind: String) -> Result<Vec<u8>, String> {
    ...
    let registered = if kind == "portrait" {
        get_video_portrait()?
            .and_then(|asset| PathBuf::from(asset.path).canonicalize().ok())
            .is_some_and(|path| path == candidate)
    } else if kind == "background" {
        get_video_background()?
            .and_then(|asset| PathBuf::from(asset.path).canonicalize().ok())
            .is_some_and(|path| path == candidate)
    } else if kind == "output" {
        list_video_jobs()?
    ...
    if !registered {
        return Err("只能读取 VoxLocal 已登记的视频素材".into());
    }
...
fn reveal_video_file(path: String) -> Result<(), String> {
    let candidate = PathBuf::from(path)
        .canonicalize()
        .map_err(|error| error.to_string())?;
    let registered = list_video_jobs()?
        .into_iter()
        .filter_map(|job| job.output_path)
        .filter_map(|path| PathBuf::from(path).canonicalize().ok())
        .any(|path| path == candidate);
    if !registered {
        return Err("只能显示 VoxLocal 已登记的视频".into());
    }
    let status = Command::new("/usr/bin/open")
        .arg("-R")
        .arg(candidate)
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            engine_status,
            prepare_ai_engine,
            public_voice_availability,
            download_public_voice,
            persist_voice_recording,
            delete_voice_recording,
            persist_voice_profile,
            list_voice_profiles,
            trash_voice_profile,
            list_trash_voice_profiles,
            restore_voice_profile,
            purge_voice_trash,
            start_synthesis,
            get_synthesis_job,
            list_synthesis_jobs,
            rename_generated_audio,
            trash_generated_audio,
            list_trashed_audio_jobs,
            restore_trashed_audio,
            purge_audio_trash,
            cancel_synthesis_job,
            read_audio_file,
            read_voice_preview_cache,
            cache_voice_preview,
            reveal_audio_file,
            video_engine_status,
            get_video_portrait,
            persist_video_portrait,
            delete_video_portrait,
            get_video_background,
            persist_video_background,
            delete_video_background,
            persist_video_audio,
            persist_ppt_slides,
            start_video_job,
            cancel_video_job,
...
mod tests {
    ...
    fn preview_cache_key_is_stable_across_frontend_and_native() {
        ...
        assert_eq!(
            ...
            Some("qwen-vivian-8667a97679f883cdbb820584.wav")
        );
    }
    ...
    fn wav_parts_are_concatenated_in_order() {
        ...
        let parts: Vec<PathBuf> = [120_i16, -240_i16]
            .iter()
            .enumerate()
            .map(|(index, value)| {
        ...
        let samples: Vec<i16> = hound::WavReader::open(&output)
            ...
            .map(Result::unwrap)
            .collect();
        // Two 1000-sample clips plus the deterministic inter-clip pause.
        assert!(samples.len() > 2000 && samples.len() < 6000);
    ...
    fn mac_system_voice_generates_real_audio_parts() {
        ...
        write_job(&SynthesisJob {
            id: id.clone(),
            source: "text".into(),
            engine_id: "macos-system".into(),
            voice_id: "warm".into(),
        ...
        .unwrap();
        let parts =
            generate_system_parts(&id, &request, &chunks, &directory, &AtomicBool::new(false))
                .unwrap();
        let output = directory.join("joined.wav");
        concat_wav(&parts, &output).unwrap();
