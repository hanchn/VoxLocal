  ...
  return <section className="page video-page">
    ...
    <div className="video-workspace">
      <div className="video-setup-card">
        ...
        <div className="video-option-grid"><label><span>画布形状</span><select value={canvas} onChange={(event) => setCanvas(event.target.value as VideoCanvas)}><option value="landscape">横屏 16:9</option><option value="square">方形 1:1</option><option value="portrait">竖屏 9:16</option><option value="circle">圆形头像</option></select></label><label><span>导出内容</span><select value={audioMode} onChange={(event) => setAudioMode(event.target.value as VideoAudioMode)}><option value="with-audio">视频 + 语音</option><option value="video-only">纯视频（无声音）</option></select></label></div>
